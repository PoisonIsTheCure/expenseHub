...
export const calculateSplitDetails = (
  ...
  switch (splitMethod) {
    case 'equal': {
      ...
      const amountPerMember = parseFloat((amount / household.members.length).toFixed(2));
...
export const calculateMonthlySpendingSummary = async (
  householdId: string,
  expenses: IExpense[]
): Promise<{ month: string; totalSpending: number }[]> => {
  ...
  // Convert the object to an array of objects for easier use in the frontend
  const monthlySpendingArray = Object.entries(monthlySpending).map(([month, totalSpending]) => ({
    month,
    totalSpending: parseFloat(totalSpending.toFixed(2)), // Round to 2 decimal places
  }));

  return monthlySpendingArray;
};

export const calculateSpendingByMember = async (
  householdId: string,
  expenses: IExpense[]
): Promise<{ userId: string; name: string; email: string; totalSpending: number }[]> => {
  const memberSpending: Map<string, { userId: string; name: string; email: string; totalSpending: number }> = new Map();

  for (const expense of expenses) {
    const userId = expense.paidBy?.toString() || expense.ownerId.toString();

    if (!memberSpending.has(userId)) {
      const user = await User.findById(userId).select('name email');
      if (user) {
        memberSpending.set(userId, {
          userId,
          name: user.name,
          email: user.email,
          totalSpending: 0,
    ...
    if (member) {
      member.totalSpending += expense.amount;
    }
  }

  // Convert the map to an array of objects
  const memberSpendingArray = Array.from(memberSpending.values()).map(member => ({
    ...member,
    totalSpending: parseFloat(member.totalSpending.toFixed(2)), // Round to 2 decimal places
  }));

  return memberSpendingArray;
};

export const calculateDebtSummary = (balances: MemberBalance[]): { [userId: string]: number } => {
    debugger;
  const debtSummary: { [userId: string]: number } = {};

  for (const balance of balances) {
    debtSummary[balance.userId] = parseFloat(balance.balance.toFixed(2)); // Round to 2 decimal places
  }
